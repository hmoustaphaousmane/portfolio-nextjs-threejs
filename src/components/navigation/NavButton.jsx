import {
  Github, Home, Linkedin, NotebookText, Palette, Phone, Twitter, User
} from 'lucide-react';
...
export const NavButton = ({ x, y, label, link, icon, newTab, labelDirection = 'right' }) => {
  return (
    <ResponsiveComponent>
      {({ size }) => size && size >= 480 ?
        <div className='absolute cusrsor-pointer z-50'
          style={{
        ...
        >
          <NavLink
            variants={item}
            href={link} target={newTab ? '_blank' : 'self'}
        ...
        '>
          ...
          mx-2 top-1/2 -translate-y-1/2 bg-background text-foreground text-sm
        ...
        : (
          <div className='w-fit cusrsor-pointer z-50'>
            <NavLink

...
const Modal = ({ onClose, toggle }) => {
  return createPortal(
    <div className='fixed inset-0 bg-background/60 backdrop-blur-sm flex items-center justify-center'>
      <div className='bg-background/20 border border-accent/30 border-solid backdrop-blur[6px] py-8 px-6 sm:px-10 sm:pc-16 rounded shadow-glass-inset text-center space-y-8'>
        <p className='font-light'>Do you like to play the background music?</p>
        <div className='flex items-center justify-center space-x-4' >
          <button onClick={toggle} className='px-4 p-y-2 border bodrder-accent/30 border-solid hover:shadow-glass-sm rounded mr-2'>Yes</button>
          <button onClick={onClose} className='px-4 p-y-2 border bodrder-accent/30 border-solid hover:shadow-glass-sm rounded'>No</button>
        </div>
...
const Sound = () => {
  ...
  return (
    <div className='fixed top-4 right-2.5 xs:right-4 z-50 group'>
      {
        showModal && <Modal onClose={() => setShowModal} toggle={toggle} />
      }
